...
public class DaskK8sHelper {
    ...
    private void createNamespace(String name, Map<String, String> labels) throws Exception {
        V1Namespace namespace = new V1NamespaceBuilder().withNewMetadata().withName(name).withLabels(labels)
                .endMetadata().build();
    ...
    private void createSecret(String namespace, String name, Map<String, byte[]> data) throws Exception {
        V1Secret secret = new V1SecretBuilder().withNewMetadata().withName(name).endMetadata().withType("Opaque")
                .withData(data).build();
    ...
    private void createService(String namespace, String name, Map<Integer, Integer> tcpPorts,
        ...
        for (int port : tcpPorts.keySet()) {
            V1ServicePort servicePort = new V1ServicePortBuilder().withProtocol("TCP").withPort(port)
                    .withTargetPort(new IntOrString(tcpPorts.get(port))).build();

            servicePorts.add(servicePort);
        }

        V1Service service = new V1ServiceBuilder().withNewMetadata().withName(name).endMetadata().withNewSpec()
                .withSelector(selector).withPorts(servicePorts).withType(serviceType).endSpec().build();

        k8sClient.getCoreApi().createNamespacedService(namespace, service, null, null, null);
    }

    private void createIngress(String namespace, String name, String serviceName, int port, String path,
            Map<String, String> annotations) throws Exception {

        V1IngressRule httpRule = new V1IngressRuleBuilder().withNewHttp().addNewPath().withNewBackend().withNewService()
                .withName(serviceName).withNewPort().withNumber(port).endPort().endService().endBackend().withPath(path)
                .withPathType("ImplementationSpecific").endPath().endHttp().build();

        V1Ingress ingress = new V1IngressBuilder().withNewMetadata().withName(name).withAnnotations(annotations)
                .endMetadata().withNewSpec().withRules(httpRule).endSpec().build();

        k8sClient.getNetApi().createNamespacedIngress(namespace, ingress, null, null, null);
    }

    private void createDaskWorkers(String namespace, int replicas, String image, String memory, int threads,
            Iterable<GenericVolume> volumes) throws Exception {

        List<V1VolumeMount> k8sVolumeMounts = new ArrayList<V1VolumeMount>();
        List<V1Volume> k8sVolumes = new ArrayList<V1Volume>();
        k8sVolumeMounts.add(new V1VolumeMountBuilder().withName("dask-certs").withMountPath("/etc/dask-certs")
                .withReadOnly(true).build());
        k8sVolumes.add(new V1VolumeBuilder().withName("dask-certs").withNewSecret().withSecretName("dask-certs")
                .endSecret().build());
        int i = 0;
        ...
        V1Container container = new V1ContainerBuilder().withName("dask-worker").withCommand("dask-worker")
                .withArgs(Arrays.asList("--worker-port", "8888", "--local-directory", "/home/idies", "--name",
                        "$(DASK_WORKER_NAME)", "--memory-limit", memory, "--nthreads", Integer.toString(threads),
                        "--resources", "MEMORY=" + memory, "tls://dask-scheduler:8786", "--tls-ca-file",
                        "/etc/dask-certs/ca.pem", "--tls-cert", "/etc/dask-certs/server-cert.pem", "--tls-key",
                        "/etc/dask-certs/server-key.pem"))
                .withEnv(Arrays.asList(
                        new V1EnvVarBuilder().withName("DASK_NODE_NAME")
                                .withValueFrom(new V1EnvVarSourceBuilder().withNewFieldRef().withApiVersion("v1")
                                        .withFieldPath("spec.nodeName").endFieldRef().build())
                                .build(),
                        new V1EnvVarBuilder().withName("DASK_WORKER_NAME")
                                .withValueFrom(new V1EnvVarSourceBuilder().withNewFieldRef().withApiVersion("v1")
                                        .withFieldPath("metadata.name").endFieldRef().build())
                                .build()))
                .withImage(image).withImagePullPolicy("Never")
                .withPorts(new V1ContainerPortBuilder().withContainerPort(8888).withProtocol("TCP").build())
                .withVolumeMounts(k8sVolumeMounts).build();

        Map<String, String> labels = new ImmutableMap.Builder<String, String>()
                .put("app.kubernetes.io/component", "dask-worker").build();

        V1Deployment deployment = new V1DeploymentBuilder().withNewMetadata().withName("dask-worker").withLabels(labels)
                .endMetadata().withNewSpec().withReplicas(replicas).withNewSelector().withMatchLabels(labels)
                .endSelector().withNewTemplate().withNewMetadata().withLabels(labels).endMetadata().withNewSpec()
                .withContainers(container).withVolumes(k8sVolumes).endSpec().endTemplate().endSpec().build();
    ...
    private void createDaskScheduler(String namespace, String image) throws Exception {
        Map<String, String> labels = new ImmutableMap.Builder<String, String>()
                .put("app.kubernetes.io/component", "dask-scheduler").build();

        V1Container container = new V1ContainerBuilder().withName("dask-scheduler").withCommand("dask-scheduler")
                .withArgs(Arrays.asList("--dashboard-prefix", "/" + namespace + "/dashboard", "--tls-ca-file",
                        "/etc/dask-certs/ca.pem", "--tls-cert", "/etc/dask-certs/server-cert.pem", "--tls-key",
                        "/etc/dask-certs/server-key.pem"))
                .withEnv(Arrays.asList(
                        new V1EnvVarBuilder().withName("DASK_NODE_NAME")
                ...
                .withImage(image).withImagePullPolicy("Never")
                .withPorts(
                        Arrays.asList(new V1ContainerPortBuilder().withContainerPort(8786).withProtocol("TCP").build(),
                                new V1ContainerPortBuilder().withContainerPort(8787).withProtocol("TCP").build()))
                .withVolumeMounts(new V1VolumeMountBuilder().withName("dask-certs").withMountPath("/etc/dask-certs")
                        .withReadOnly(true).build())
                .build();

        V1Deployment deployment = new V1DeploymentBuilder().withNewMetadata().withName("dask-scheduler")
                .withLabels(labels).endMetadata().withNewSpec().withReplicas(1).withNewSelector()
                .withMatchLabels(labels).endSelector().withNewTemplate().withNewMetadata().withLabels(labels)
                .endMetadata().withNewSpec().withContainers(container).withVolumes(new V1VolumeBuilder()
                        .withName("dask-certs").withNewSecret().withSecretName("dask-certs").endSecret().build())
    ...
    public void createDaskCluster(DaskCluster cluster, int workers, String memory, int threads,
            Iterable<GenericVolume> volumes) throws Exception {
        ...
        X509Certificate server = SecurityUtils.createSingedCert(serverKeyPair, ca, caKeyPair.getPrivate(), "CN=Server");

        // Generate client certificate
        KeyPair clientKeyPair = SecurityUtils.createKeyPair();
        X509Certificate client = SecurityUtils.createSingedCert(clientKeyPair, ca, caKeyPair.getPrivate(), "CN=Client");

        createNamespace(externalRef, labels);

        createSecret(externalRef, "dask-certs",
                new ImmutableMap.Builder<String, byte[]>().put("ca.pem", SecurityUtils.getCertPem(ca))
                        ...
                        .put("client-key.pem", SecurityUtils.getPrivateKeyPem(clientKeyPair.getPrivate())).build());

        createService(externalRef, "dask-scheduler",
                new ImmutableMap.Builder<Integer, Integer>().put(8786, 8786).build(),
                new ImmutableMap.Builder<String, String>().put("app.kubernetes.io/component", "dask-scheduler").build(),
                ServiceType.NODE_PORT);

        createService(externalRef, "dask-dashboard",
                new ImmutableMap.Builder<Integer, Integer>().put(8787, 8787).build(),
                new ImmutableMap.Builder<String, String>().put("app.kubernetes.io/component", "dask-scheduler").build(),
                ServiceType.CLUSTER_IP);

        createIngress(externalRef, "dask-dashboard", "dask-dashboard", 8787, "/" + externalRef + "/dashboard",
                new ImmutableMap.Builder<String, String>().put("nginx.ingress.kubernetes.io/ssl-redirect", "false")
                        .build());

        String image = cluster.getImage().getDockerRef();

        createDaskScheduler(externalRef, image);

        createDaskWorkers(externalRef, workers, image, memory, threads, volumes);
    }
    ...
    public DaskClusterInfo getDaskClusterInfo(String refId, boolean connectionInfo) throws Exception {
        V1NamespaceList namespaces = k8sClient.getCoreApi().listNamespace(null, false, null,
                "status.phase!=Terminating,metadata.name=" + refId, null, 1, null, null, 10, false);
    ...
    public List<DaskClusterInfo> getDaskClusterInfo(UserInfo user) throws Exception {
        V1NamespaceList namespaces = k8sClient.getCoreApi().listNamespace(null, false, null,
                "status.phase!=Terminating", "sciserver.org/user-id=" + user.getUserId(), -1, null, null, 10, false);
    ...
    private DaskConnectionInfo getDaskConnectionInfo(V1Namespace namespace) throws Exception {
        ...
        V1ServiceList services = k8sClient.getCoreApi().listNamespacedService(namespace.getMetadata().getName(), null,
                false, null, "metadata.name=dask-scheduler", null, 1, null, null, 10, false);
        ...
        V1SecretList secrets = k8sClient.getCoreApi().listNamespacedSecret(namespace.getMetadata().getName(), null,
                false, null, "metadata.name=dask-certs", null, 1, null, null, 10, false);
