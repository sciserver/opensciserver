...
public class RACMVolumeManager extends ContainerManager implements VolumeManager {
    ...
    public VolumeContainer createContainer(String userId, Node node, VolumeImage image) throws Exception {
         ...
         * container.setNodeId(node.getId()); container.setVolumeImageId(image.getId());
         * container.setUserId(userId); container.register();
         * 
         ...
         * container.setStatus(ContainerStatus.CREATED); container.update();
         * 
         * return container;
         */
     ...
     * 
     * ObjectNode result = factory.objectNode().put("HostName", "").put("User",
     * "").put("Memory", 0) .put("MemorySwap", 0).put("AttachStdin",
     * false).put("AttachStdout", true).put("AttachStderr", true)
     * .putNull("PortSpecs").put("Privileged", false).put("Tty",
     * false).put("OpenStdin", false) .put("StdinOnce",
     * false).putNull("Env").putNull("Dns").put("Image", image.getDockerRef())
     * .put("WorkingDir", "");
     * 
     * result.putObject("Volumes").putObject(image.getContainerPath());
     * 
     * result.putArray("VolumesFrom"); result.putArray("Cmd").add("/bin/true");
     * 
     * ObjectNode hostConfig = result.putObject("HostConfig");
     * hostConfig.putArray("Binds") .add(String.format(image.getLocalPathTemplate(),
     * userId) + ":" + image.getContainerPath() + ":rw");
