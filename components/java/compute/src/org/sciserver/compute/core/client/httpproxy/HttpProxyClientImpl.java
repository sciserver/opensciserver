...
public class HttpProxyClientImpl implements HttpProxyClient {
	...
	public void setProxy(String route, String target, String userid, long containerid) throws Exception {
		...
		String jUserId = factory.textNode(userid).toString();
		String jContainerId = factory.numberNode(containerid).toString();
		String jTarget = factory.textNode(target).toString();
		...
		try {
			...
			requestAsString = requestAsString
					.replace(" ", "")
					.replace('`', '"')
					.replace("$USER_ID", jUserId)
					.replace("$CONTAINER_ID", jContainerId)
					.replace("$TARGET", jTarget);
