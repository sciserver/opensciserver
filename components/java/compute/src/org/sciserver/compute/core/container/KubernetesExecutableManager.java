...
public class KubernetesExecutableManager extends ContainerManager implements ExecutableManager {
    ...
    public KubernetesExecutableManager(Image image) {
        super(image);
    }
    ...
    public ExecutableContainer createContainer(String name, String description, AuthenticatedUser user,
        ...
        try (RegistryLock lock = new RegistryLock(registry)) {
            ...
            for (PublicVolume pvol : registryPubVolumes) {
                String cpath = "/home/idies/workspace/" + pvol.getName();
                addVolumes(pvol.getDockerRef(), cpath, !pvol.isWritable(), volumes, volumeMounts);
            }
