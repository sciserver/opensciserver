...
public abstract class MetadataObject extends MetadataElement implements IMetadataObjectContainer, Navigable<MetadataObject> {
  //~ Constants --------------------------------------------------------------------------------------------------------
   ...
   */
  private static AtomicLong PURELY_TRANSIENT_ID = new AtomicLong(-1l);
  /**
  ...
  public final ObjectClassType getClassMetaData() {
    return MetaModelFactory.getInstance().getObjectClassType(getClassName());
  }
   ...
   */
  public Identity getIdentityField() {
  ...
  /**
   * Indicates if this object is a transient copy of a persistent object.<br/>
   * @return true if this object is transient copy of a persistent object, false otherwise
   */
  public boolean isTransientCopy() {
	...
	public String getIvoId() {
		String ivoId;
		if (isTransientCopy()) {
			ivoId = IVOIdPrefix + getUtype() + "/" + "M" + Long.toString(-getId().longValue());
		} else {
			ivoId = IVOIdPrefix + getUtype() + "/" + Long.toString(getId().longValue());
		}
		return ivoId;
	}
  ...
  public String getXmlId() {
    String ivoId;
    if (isTransientCopy()) {
      ivoId = getClass().getSimpleName() + "_" + "M" + Long.toString(-getId().longValue());
    } else {
      ivoId = getClass().getSimpleName() + "_" + Long.toString(getId().longValue());
    }
    return ivoId;
  }
  ...
  protected final MetadataObject resolve(final Reference pReference, final Class<? extends MetadataObject> type) {
    throw new UnsupportedOperationException("resolve method called on a MetadataObject. This should only be done if XML serialization was used.");
  }
  ...
  /**
   * Generate a new negative long value (starting from -1)
   * @return negative long value
   */
  private final Long newId() {
    return Long.valueOf(PURELY_TRANSIENT_ID.decrementAndGet());
  }
  ...
  public TransientObjectManager getTom() {
		return getContainer().getTom();
	}

  protected void setTom(TransientObjectManager tom) {
		((MetadataObject)getContainer()).setTom(tom);
	}
  public abstract IMetadataObjectContainer getContainer();
