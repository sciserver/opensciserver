...
-- currently queued/running jobs show up with a negative ranking. 
-- The most negative ranking in a compute domain is the one started earliest
...
,      id as jobId,status,submitterId, computeDomainId, submitTime, startedTime 
  from job where status between 2 and 16 and computedomainid=@computeDomainId
...
-- refresh the function retrieving info on all queues
exec sp_refreshsqlmodule 'racm.allJobsQueues'
